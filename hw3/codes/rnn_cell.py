...
class RNNCell(base_layer.Layer):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        if scope is not None:
            ...
            with vs.variable_scope(vs.get_variable_scope(), custom_getter=self._rnn_get_variable):
...
class BasicLSTMCell(RNNCell):
    ...
    def call(self, inputs, state):
        ...
        g = _linear([inputs, h], 4 * self._num_units, bias=True)
        i, f, o, cc = array_ops.split(value=g, num_or_size_splits=4, axis=1)
        new_c = sigmoid(f + self._forget_bias) * c + sigmoid(i) * self._activation(cc)
