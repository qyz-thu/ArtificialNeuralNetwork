...
from mlp.load_data import load_cifar_2d
...
tf.app.flags.DEFINE_integer("hidden_size", 768, "hidden layer size in training")
tf.app.flags.DEFINE_float("drop_rate", 0.3, "drop out rate")
...
with tf.Session() as sess:
    ...
    if FLAGS.is_train:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            train_acc, train_loss, acc_, loss_, acc_val, loss_val = train_epoch(mlp_model, sess, X_train, y_train)  # Complete the training process
            train_acc_list += acc_
        ...
        with open("result.txt", 'w') as f:
            f.write("epochs: %d  iterations: %d\n" % (FLAGS.num_epochs, FLAGS.num_epochs * 50000 / FLAGS.batch_size))
