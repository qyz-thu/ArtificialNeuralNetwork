...
from cnn.load_data import load_cifar_4d
...
tf.app.flags.DEFINE_integer("hidden_size1", 128, "hidden size 1")
tf.app.flags.DEFINE_integer("hidden_size2", 32, "hidden size 2")
tf.app.flags.DEFINE_float("drop_rate", 0.6, "drop out rate")
...
tf.app.flags.DEFINE_integer("stop_times", 5, "number of epoch without valid-loss before stop training")
...
def train_epoch(model, sess, X, y, X_val, y_val): # Training Process
    loss, acc = 0.0, 0.0
    ...
    while st < len(X) and ed <= len(X):
        ...
        print("finished %d iterations" % times)
        if (times + 1) % 10 == 0:
            val_acc, val_loss = valid_epoch(model, sess, X_val, y_val)
    ...
    loss /= times
    acc /= times
    return acc, loss, acc_list, loss_list, val_acc_list, val_loss_list
...
with tf.Session() as sess:
    ...
    if FLAGS.is_train:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            train_acc, train_loss, acc_, loss_, acc_val, loss_val = train_epoch(cnn_model, sess, X_train, y_train, X_val, y_val)
            train_acc_list += acc_
            train_loss_list += loss_
            val_acc_list += acc_val
            val_loss_list += loss_val
        ...
        np.save("val_acc.npy", val_acc_list)
        np.save("val_loss.npy", val_loss_list)
        with open("result.txt", 'w') as f:
            f.write("epochs: %d  iterations: %d\n" % (FLAGS.num_epochs, FLAGS.num_epochs * 40000 / FLAGS.batch_size))
            f.write("time used: %d s\n" % (time.time() - initial_time))
            f.write("best validation accuracy: %.4f\n" % best_val_acc)
            f.write("final test accuracy: %.4f" % test_acc)
